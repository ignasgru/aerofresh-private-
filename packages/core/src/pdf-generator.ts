...
export interface AircraftReportData {
  tail: string;
  make: string;
  model: string;
  year: number;
...
export class PDFGenerator {
  ...
  async generateAircraftReport(data: AircraftReportData): Promise<ArrayBuffer> {
  ...
  private createAircraftReportHTML(data: AircraftReportData): string {
  ...
  private async convertHTMLToPDF(html: string): Promise<ArrayBuffer> {
...
%%EOF`;
  ...
  private getRiskAssessment(score: number): string {
    if (score >= 70) return 'This aircraft has a high risk score due to multiple accidents or open ADs.';
    if (score >= 40) return 'This aircraft has a moderate risk score. Review safety history.';
