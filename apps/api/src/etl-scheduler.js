 ...
 */
function getJobBySchedule(cron) {
 ...
 * Execute an ETL job
 */
async function executeETLJob(job, env) {
 ...
 */
async function executeLightweightETL(job, env) {
 ...
 */
async function fetchLiveADS_B(env) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            await env.RAW_DATA.put(`adsb/${new Date().toISOString().split('T')[0]}/${Date.now()}.json`, JSON.stringify(data));
 ...
 */
async function handleManualETL(request, env, path) {
 ...
 */
async function handleETLStatus(env) {
    const status = {
        timestamp: new Date().toISOString(),
        jobs: {}
    };
 ...
 */
async function handleQueueConsumer(request, env) {
