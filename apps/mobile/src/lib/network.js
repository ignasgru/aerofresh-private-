...
export const demoData = {
    ...
    livePositions: (limit = 20, minutes = 30) => {
...
export async function apiCall(endpoint, demoDataGenerator, options = {}) {
    // Check health first
...
export const api = {
    ...
    aircraftSummary: (tail) => apiCall(`/api/aircraft/${encodeURIComponent(tail)}/summary`, () => ({
        ...demoData.aircraft(),
        tail: tail.toUpperCase(),
        source: 'demo'
    })),
    // Get live positions - with real ADS-B data support
    livePositions: (limit = 20, minutes = 30) => apiCall(`/api/tracking/live?limit=${limit}&minutes=${minutes}`, () => ({
    ...
    comprehensiveReport: (tail) => apiCall(`/api/aircraft/${encodeURIComponent(tail)}/history`, () => ({
