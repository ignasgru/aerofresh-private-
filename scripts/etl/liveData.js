...
function parseOpenSkyState(state) {
    ...
    return {
        ...
        origin_country: state[2] || undefined,
        time_position: state[3] || undefined,
        last_contact: state[4] || undefined,
        ...
        sensors: Array.isArray(state[12]) ? state[12] : undefined,
        geo_altitude: state[13] || undefined,
        squawk: state[14] || undefined,
        spi: state[15] || false,
        position_source: state[16] || undefined
    };
}
// Fetch live aircraft data from OpenSky Network
export async function fetchLiveAircraftData() {
    ...
    try {
        ...
        for (const state of data.states) {
            try {
                ...
                if (!position.latitude || !position.longitude || !position.baro_altitude)
                    continue;
...
async function convertIcaoToTail(icao24) {
...
async function createSampleLiveData() {
...
export async function startLiveDataService() {
